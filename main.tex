...
\newcommand{\etz}[1]{\todo[inline,color=pink]{#1  -\textit{ae}}}
...
Alternative methods have been proposed. One prominent example is the so-called HDI+ROPE decision rule (henceforth, \hdr{}) introduced by \citeA{Kruschke2011,Kruschke2013} as a superseding alternative to classical NHST.  
...
\noindent This example serves to highlight the critical weakness of the \hdr{}: whether the null hypothesis is rejected depends on \rev{how one decides to portray their results}{what is essentially an historical and cultural accident}.  
That is, if the individuals who collected the data had come from a different tradition then the same data would have led them to different conclusions.
Avery and Blair are able to reject the null hypothesis \rev{when the results are presented}{due to their statistically arbitrary preference for a parameterization} in terms of probability or log-odds of success, respectively, but Cassidy must withhold judgment \rev{when}{owing to their preference for} framing \rev{the}{} results in terms of odds of success.  With the same model, the same prior information, and the same data, the triplets come to different conclusions. In other words, their conclusions do not \textit{cohere}. 

\rev{}{It is important to emphasize here that incoherence is a constant property of the \hdr{}.  Even in cases where one chooses and stays with a single parameterization of the model---that is, one never transforms parameters from one space to another---the issue persists: \textit{Any} choice of parameter space is statistically arbitrary and conclusions from the \hdr{} depend critically on that choice.}\\

\noindent While the \rev{above}{hamster} scenario is of course a fiction, this is not an unusual or selectively presented pattern of data -- the numerical values in this section were taken from Figure~1 in the paper that first described the \hdr{} procedure \cite{Kruschke2011}.
...
Indeed, as \citeA{bernardo2005} points out, ``\textit{any} feature of the posterior which is not invariant under reparametrization is completely illusory, since the parametrization is arbitrary'' (p.~374, emphasis original).
...
\rev{}{Note that the incoherence problem does not involve moving from one parameterization to another -- the changing choice of parameters simply serves to illustrate the fact that the results of the \hdr{} depend on this statistically arbitrary choice.}
...
\citeA{Kruschke2010Book} provides code to produce HDIs from MCMC output and produce figures summarizing the results. In Figure~\ref{fig:rcode} we provide code that can be used to replace the HDIs with quantile-based intervals. This simple fix brings the procedure back to a state of coherence while allowing the user to continue using the computational framework provided by \citeA{Kruschke2010Book} with minimal interruption.
...
\caption{R code to compute quantile intervals instead of HDIs.  This function has the same interface and can be used in the same way as the \texttt{HDIofMCMC} function provided by \protect\citeA[p.~628-629]{Kruschke2010Book}.}
    \label{fig:rcode}
\end{figure}
...
\begin{table}
\caption{A selection of results of the logistic regression analysis from Newton et al. (2018). \rev{}{The key statistic is the odds ratio $\theta$.}}\label{tab:newton}
...
However, we prefer probability-based intervals over ``intrinsic'' intervals, if for no other reason than that they are computationally and conceptually simpler and thus easier to adopt in practice. \rev{}{More broadly, for the reasons we discuss in this paper and those presented by \citeauthor{bernardo2005} and others, we do not believe HDIs are useful tools for Bayesians.}
