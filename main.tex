...
%For notational convenience, if the $\alpha$ being used is the standard .95 we will simply write ``$\HDI$'' without the superscript. 
...
%These results were explained as a byproduct of the lower-than-expected prevalence of these behaviors in the overall sample (i.e., $5\%$ of the total students). 
%
The authors then implemented an \hdr{} to quantify how much evidence the data provided in favor or against the null hypothesis. %This decision was motivated by the fact that Bayesian hypothesis testing techniques are known to quantify the evidence in favor of both the null and the alternative hypothesis. 
%The \hdr{} was implemented using a multi-level Bayesian logistic regression model to account for the probability of cannabis usage and related harms at a given moment in time for participants in the control and intervention group.
...
%\newpage\onecolumn
%\appendix\section{R code for the quantile interval}
